...
public class Music {
    ...
    private void establishSynthesizer(final int numCh, final int instr) {
        final int[] instruments = new int[numCh];
    ...
    public java.util.List<Instrument> availableInstruments() {
        ...
        final Instrument[] availInst = synth.getAvailableInstruments();
        final int numInst = availInst.length;
    ...
    public void playChord(final Chord chord, final int defaultDuration) {
        ...
        /** Note durations */
        final int[] noteDurations = new int[chord.numVoices()];
        /** Chord iterator */
        final Iterator<Note> chordIt = chord.iterator();
        ...
        while (chordIt.hasNext()) {
            ...
            if (note.equals(Note.NULL_NOTE)) {
                ...
                channels[note.channel()].noteOff(
                    prev.pitch(), prev.velocityOn());
                channels[note.channel()].noteOn(
                    note.pitch(), note.velocityOn());
    ...
    public void playTune(final Tune tune) {
        ...
        Chord chord;
        /** Current duration */
        final int duration = STD_DURATION;
        /** Tune iterator */
        final Iterator<Chord> tuneIt = tune.iterator();
        int line = 0;
    ...
    public void silence() {
        Note note;
        final int numNotes = noteHistory.size();
